...
void ui_S1Main_screen_init(void)
{
    ...
    lv_style_set_line_width(&style_line_mn, 5);
    lv_style_set_line_color(&style_line_mn, lv_palette_main(LV_PALETTE_RED));
    lv_style_set_line_rounded(&style_line_mn, false);
...
void set_hands(time_t now)
{
    ...
    calculate_line_endpoints(hour_hands_points, 120, angle_degrees);

    // calculate the angle of the minute hand
    angle_degrees = (minute * 360) / 60;
    calculate_line_endpoints(minute_hand_points, 175, angle_degrees);

    // calculate the angle of the second hand
    angle_degrees = (second * 360) / 60;
    calculate_line_endpoints(second_hand_points, 200, angle_degrees);
